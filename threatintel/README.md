# Threat Intelligence

A curated collection of threat intelligence tools, platforms, and resources for analyzing malware, tracking APTs, and understanding cyber threats.

**Total Tools:** 103

---

## android-malware

**Repository:** [https://github.com/ashishb/android-malware](https://github.com/ashishb/android-malware)

**Author:** ashishb

**Last Updated:** 2025-12-30

**Description:** 1. Largest open collection of Android malware samples 2. Live samples - use them at your peril 3. Collected from several sources/mailing lists 4. Contributions are welcome - please create a new directory for every sample type, add a README file, and samples in that directory

---

## APTnotes

**Repository:** [https://github.com/kbandla/APTnotes](https://github.com/kbandla/APTnotes)

**Author:** kbandla

**Last Updated:** 2024-01-01

**Description:** This is a repository for various publicly-available documents and notes related to APT, sorted by year. For malware sample hashes, please see the individual reports.

---

## APTSimulator

**Repository:** [https://github.com/NextronSystems/APTSimulator](https://github.com/NextronSystems/APTSimulator)

**Author:** NextronSystems

**Last Updated:** 2025-09-23

**Description:** Important: Don't just ZIP download or clone the repo if you don't plan to develop some test. Use the package in the [release](https://github.com/Neo23x0/APTSimulator/releases) which contains a packaged version with encrypted archives that don't contain cleartext samples and tools.

---

## AutoYara

**Repository:** [https://github.com/NeuromorphicComputationResearchProgram/AutoYara](https://github.com/NeuromorphicComputationResearchProgram/AutoYara)

**Author:** NeuromorphicComputationResearchProgram

**Last Updated:** 2025-10-08

**Description:** This is the java code implementing the AutoYara algorithm, from out paper [_Automatic Yara Rule Generation Using Biclustering_](https://arxiv.org/abs/2009.03779). Given a set up input files that belong to a given malware family, AutoYara can create [Yara](https://yara.readthedocs.io/en/stable/) r...

---

## awesome-malware

**Repository:** [https://github.com/fabacab/awesome-malware](https://github.com/fabacab/awesome-malware)

**Author:** fabacab

**Last Updated:** 2021-03-14

**Description:** > A curated collection of awesome malware, botnets, and other post-exploitation tools.

---

## awesome-malware-analysis

**Repository:** [https://github.com/rshipp/awesome-malware-analysis](https://github.com/rshipp/awesome-malware-analysis)

**Author:** rshipp

**Last Updated:** 2024-05-30

**Description:** A curated list of awesome malware analysis tools and resources. Inspired by [awesome-python](https://github.com/vinta/awesome-python) and [awesome-php](https://github.com/ziadoz/awesome-php).

---

## awesome-pcaptools

**Repository:** [https://github.com/caesar0301/awesome-pcaptools](https://github.com/caesar0301/awesome-pcaptools)

**Author:** caesar0301

**Last Updated:** 2025-09-03

**Description:** Introduction

---

## awesome-threat-detection

**Repository:** [https://github.com/0x4D31/awesome-threat-detection](https://github.com/0x4D31/awesome-threat-detection)

**Author:** 0x4D31

**Last Updated:** 2026-01-05

**Description:** > A curated list of awesome threat detection and hunting resources

---

## awesome-threat-intelligence

**Repository:** [https://github.com/hslatman/awesome-threat-intelligence](https://github.com/hslatman/awesome-threat-intelligence)

**Author:** hslatman

**Last Updated:** 2026-01-19

**Description:** A curated list of awesome Threat Intelligence resources

---

## awesome-yara

**Repository:** [https://github.com/InQuest/awesome-yara](https://github.com/InQuest/awesome-yara)

**Author:** InQuest

**Last Updated:** 2025-03-26

**Description:**  

---

## broyara

**Repository:** [https://github.com/hempnall/broyara](https://github.com/hempnall/broyara)

**Author:** hempnall

**Last Updated:** 2014-12-09

**Description:** integrating bro into yara

---

## Burp-Yara-Rules

**Repository:** [https://github.com/codewatchorg/Burp-Yara-Rules](https://github.com/codewatchorg/Burp-Yara-Rules)

**Author:** codewatchorg

**Last Updated:** 2022-01-26

**Description:** Burp-Yara-Rules

---

## CapTipper

**Repository:** [https://github.com/omriher/CapTipper](https://github.com/omriher/CapTipper)

**Author:** omriher

**Last Updated:** 2020-10-02

**Description:** [Logo]: http://4.bp.blogspot.com/-uuRE1KkS5Jo/Vb8j-cfEuHI/AAAAAAAAeY4/MltsTu7jG5E/s1600/CapTipper_logo.png

---

## CCCS-Yara

**Repository:** [https://github.com/CybercentreCanada/CCCS-Yara](https://github.com/CybercentreCanada/CCCS-Yara)

**Author:** CybercentreCanada

**Last Updated:** 2025-05-12

**Description:** The [CCCS YARA Specification](https://github.com/CybercentreCanada/CCCS-Yara/blob/master/CCCS_YARA.yml) has been created to define and validate the style and format of YARA rule metadata. It comes with a cli which allow you to validate and generate metadata automatically (such as unique id, rule ...

---

## CDI_yara

**Repository:** [https://github.com/CyberDefenses/CDI_yara](https://github.com/CyberDefenses/CDI_yara)

**Author:** CyberDefenses

**Last Updated:** 2017-07-11

**Description:** A collection of YARA rules for public use.  Built from information in intelligence profiles, dossiers and file work.

---

## cti-python-stix2

**Repository:** [https://github.com/oasis-open/cti-python-stix2](https://github.com/oasis-open/cti-python-stix2)

**Author:** oasis-open

**Last Updated:** 2026-02-12

**Description:** |Build_Status| |Coverage| |Version| |Downloads_Badge| |Documentation_Status|

---

## dnYara

**Repository:** [https://github.com/airbus-cert/dnYara](https://github.com/airbus-cert/dnYara)

**Author:** airbus-cert

**Last Updated:** 2022-05-25

**Description:** __| |_ __/\_/\__ _ _ __ __ _ / _` | '_ \_ _/ _` | '__/ _` | | (_| | | | / \ (_| | | | (_| | \__,_|_| |_\_/\__,_|_|  \__,_|

---

## FindYara

**Repository:** [https://github.com/OALabs/FindYara](https://github.com/OALabs/FindYara)

**Author:** OALabs

**Last Updated:** 2021-12-28

**Description:**  

---

## fsYara

**Repository:** [https://github.com/filescanio/fsYara](https://github.com/filescanio/fsYara)

**Author:** filescanio

**Last Updated:** 2026-02-10

**Description:** A collection of curated YARA rules used as part of the Filescan.io service

---

## go-yara

**Repository:** [https://github.com/hillu/go-yara](https://github.com/hillu/go-yara)

**Author:** hillu

**Last Updated:** 2025-07-01

**Description:** Go bindings for [YARA](https://virustotal.github.io/yara/), staying as close as sensible to the library's C-API while taking inspiration from the `yara-python` implementation.

---

## HSN-Capture-HPC-NG

**Repository:** [https://github.com/CERT-Polska/HSN-Capture-HPC-NG](https://github.com/CERT-Polska/HSN-Capture-HPC-NG)

**Author:** CERT-Polska

**Last Updated:** 2011-12-19

**Description:** HoneySpider Network Capture-HPC NG source code repository. Project is released under GPL 2.0 license.

---

## Hyara

**Repository:** [https://github.com/hyuunnn/Hyara](https://github.com/hyuunnn/Hyara)

**Author:** hyuunnn

**Last Updated:** 2024-10-18

**Description:** > Hyara is plugin that provides convenience when writing yararule.

---

## ida-yara-processor

**Repository:** [https://github.com/bnbdr/ida-yara-processor](https://github.com/bnbdr/ida-yara-processor)

**Author:** bnbdr

**Last Updated:** 2019-01-22

**Description:** Loader and processor for YARA's compiled rule format

---

## ida_yara

**Repository:** [https://github.com/alexander-hanel/ida_yara](https://github.com/alexander-hanel/ida_yara)

**Author:** alexander-hanel

**Last Updated:** 2018-09-04

**Description:** A python script that can be used to scan data within in an IDB using Yara. The code mimics [IDA](https://www.hex-rays.com/products/ida/support/idadoc/284.shtml)'s `find_text` and `find_binary`. The script creates the Yara signature based off of the search and its search flags.

---

## ioc_writer

**Repository:** [https://github.com/mandiant/ioc_writer](https://github.com/mandiant/ioc_writer)

**Author:** mandiant

**Last Updated:** 2017-12-11

**Description:** The source code in this package is made available under the terms of the Apache License , Version 2.0. See the "LICENSE " file for more information.

---

## iocs

**Repository:** [https://github.com/fireeye/iocs](https://github.com/fireeye/iocs)

**Author:** fireeye

**Last Updated:** 2016-06-02

**Description:** Readme for IOCs to accompany FireEye blog and other public posts.

---

## IOCs

**Repository:** [https://github.com/advanced-threat-research/IOCs](https://github.com/advanced-threat-research/IOCs)

**Author:** advanced-threat-research

**Last Updated:** 2021-08-04

**Description:** Copyright © McAfee 2020

---

## java2yara

**Repository:** [https://github.com/fxb-cocacoding/java2yara](https://github.com/fxb-cocacoding/java2yara)

**Author:** fxb-cocacoding

**Last Updated:** 2021-02-15

**Description:** A minimal library to generate YARA rules from JAVA with maven support.

---

## javascript-malware-collection

**Repository:** [https://github.com/HynekPetrak/javascript-malware-collection](https://github.com/HynekPetrak/javascript-malware-collection)

**Author:** HynekPetrak

**Last Updated:** 2024-07-23

**Description:** A collection of almost 40.000 Javascript malware samples.

---

## language-yara

**Repository:** [https://github.com/blacktop/language-yara](https://github.com/blacktop/language-yara)

**Author:** blacktop

**Last Updated:** 2021-09-04

**Description:** Atom syntax highlighting for [Yara](https://github.com/VirusTotal/yara) Rule files.

---

## libyara.NET

**Repository:** [https://github.com/microsoft/libyara.NET](https://github.com/microsoft/libyara.NET)

**Author:** microsoft

**Last Updated:** 2026-01-08

**Description:** A .NET wrapper for libyara that provides a simplified API for developing tools in C# and PowerShell. This library targets .NET 4.6.2.

---

## lw-yara

**Repository:** [https://github.com/Hestat/lw-yara](https://github.com/Hestat/lw-yara)

**Author:** Hestat

**Last Updated:** 2021-03-03

**Description:** Yara rulset based on php shells and other webserver malware.

---

## malware-ioc

**Repository:** [https://github.com/eset/malware-ioc](https://github.com/eset/malware-ioc)

**Author:** eset

**Last Updated:** 2026-01-30

**Description:** No README found

---

## malware-persistence

**Repository:** [https://github.com/Karneades/malware-persistence](https://github.com/Karneades/malware-persistence)

**Author:** Karneades

**Last Updated:** 2025-10-03

**Description:** **An awesome list with only the links is found on [awesome-malware-persistence](https://github.com/Karneades/awesome-malware-persistence).**

---

## malware-signatures

**Repository:** [https://github.com/citizenlab/malware-signatures](https://github.com/citizenlab/malware-signatures)

**Author:** citizenlab

**Last Updated:** 2016-11-17

**Description:** This repository contains Yara signatures developed by [Citizen Lab](https://citizenlab.org/) during research on targeted attacks on civil society organizations.

---

## mkYARA

**Repository:** [https://github.com/fox-it/mkYARA](https://github.com/fox-it/mkYARA)

**Author:** fox-it

**Last Updated:** 2019-12-16

**Description:** Writing YARA rules based on executable code within malware can be a tedious task. An analyst cannot simply copy and paste raw executable code into a YARA rule, because this code contains variable values, such as memory addresses and offsets. The analyst has to disassemble the code and wildcard al...

---

## node-aptoide

**Repository:** [https://github.com/dweinstein/node-aptoide](https://github.com/dweinstein/node-aptoide)

**Author:** dweinstein

**Last Updated:** 2015-07-31

**Description:** Download APKs from [aptoide](http://m.aptoide.com/).

---

## node-yara

**Repository:** [https://github.com/nospaceships/node-yara](https://github.com/nospaceships/node-yara)

**Author:** nospaceships

**Last Updated:** 2019-06-11

**Description:** This module implements [YARA][yara] bindings for [Node.js][nodejs].

---

## ocaml-yara

**Repository:** [https://github.com/elastic/ocaml-yara](https://github.com/elastic/ocaml-yara)

**Author:** elastic

**Last Updated:** 2021-10-19

**Description:** No description available

---

## OCyara

**Repository:** [https://github.com/bandrel/OCyara](https://github.com/bandrel/OCyara)

**Author:** bandrel

**Last Updated:** 2018-10-30

**Description:** The OCyara module performs OCR (Optical Character Recognition) on image files and scans them for matches to Yara rules.  OCyara also can process images embedded in PDF files. For more information about Yara, visit

---

## Open-Source-YARA-rules

**Repository:** [https://github.com/mikesxrs/Open-Source-YARA-rules](https://github.com/mikesxrs/Open-Source-YARA-rules)

**Author:** mikesxrs

**Last Updated:** 2024-02-07

**Description:** No README found

---

## pcaptoparquet

**Repository:** [https://github.com/nokia/pcaptoparquet](https://github.com/nokia/pcaptoparquet)

**Author:** nokia

**Last Updated:** 2025-11-26

**Description:** This is a package for converting pcap files primarily to parquet format. CSV and JSON formats are also supported.

---

## python-iocextract

**Repository:** [https://github.com/InQuest/python-iocextract](https://github.com/InQuest/python-iocextract)

**Author:** InQuest

**Last Updated:** 2024-08-28

**Description:** iocextract

---

## shotgunyara

**Repository:** [https://github.com/darienhuss/shotgunyara](https://github.com/darienhuss/shotgunyara)

**Author:** darienhuss

**Last Updated:** 2022-08-29

**Description:** Given a string, either:

---

## sigma

**Repository:** [https://github.com/Neo23x0/sigma](https://github.com/Neo23x0/sigma)

**Author:** Neo23x0

**Last Updated:** 2026-02-10

**Description:** <a href="https://sigmahq.io/"> 

---

## Sigma2KQL

**Repository:** [https://github.com/Khadinxc/Sigma2KQL](https://github.com/Khadinxc/Sigma2KQL)

**Author:** Khadinxc

**Last Updated:** 2026-02-02

**Description:** Sigma Queries turned into KQL for Defender and Microsoft Sentinel using [pysigma-backend-KQL-backend](https://github.com/AttackIQ/pySigma-backend-kusto/tree/main)

---

## Sigma2SPL

**Repository:** [https://github.com/Khadinxc/Sigma2SPL](https://github.com/Khadinxc/Sigma2SPL)

**Author:** Khadinxc

**Last Updated:** 2026-02-08

**Description:** Sigma Queries turned into SPL for Splunk Enterprise and Enterprise Security using pysigma - Automated [pysigma-backend-SPL-backend](https://github.com/SigmaHQ/pySigma-backend-splunk)

---

## TerraSigma

**Repository:** [https://github.com/Khadinxc/TerraSigma](https://github.com/Khadinxc/TerraSigma)

**Author:** Khadinxc

**Last Updated:** 2026-02-08

**Description:** __Terraform-converted Sigma rules for deployment to Microsoft Sentinel.__

---

## threat_note

**Repository:** [https://github.com/defpoint/threat_note](https://github.com/defpoint/threat_note)

**Author:** defpoint

**Last Updated:** 2023-12-31

**Description:** Please check out https://www.threatnote.io for the latest threatnote

---

## ThreatHunting-Keywords-yara-rules

**Repository:** [https://github.com/mthcht/ThreatHunting-Keywords-yara-rules](https://github.com/mthcht/ThreatHunting-Keywords-yara-rules)

**Author:** mthcht

**Last Updated:** 2025-05-11

**Description:** Yara rules for Threat Hunting sessions

---

## ThreatIngestor

**Repository:** [https://github.com/InQuest/ThreatIngestor](https://github.com/InQuest/ThreatIngestor)

**Author:** InQuest

**Last Updated:** 2023-11-02

**Description:**  ThreatIngestor

---

## ThreatKB

**Repository:** [https://github.com/InQuest/ThreatKB](https://github.com/InQuest/ThreatKB)

**Author:** InQuest

**Last Updated:** 2024-01-29

**Description:**  

---

## ThreatMapper

**Repository:** [https://github.com/deepfence/ThreatMapper](https://github.com/deepfence/ThreatMapper)

**Author:** deepfence

**Last Updated:** 2025-07-24

**Description:** <a href="https://trendshift.io/repositories/171" target="_blank"></a>

---

## ThreatTracker

**Repository:** [https://github.com/michael-yip/ThreatTracker](https://github.com/michael-yip/ThreatTracker)

**Author:** michael-yip

**Last Updated:** 2015-03-09

**Description:** ThreatTracker is a IOC tracker written in Python.

---

## vscode-yara

**Repository:** [https://github.com/infosec-intern/vscode-yara](https://github.com/infosec-intern/vscode-yara)

**Author:** infosec-intern

**Last Updated:** 2023-01-07

**Description:** Language support for the YARA pattern matching language

---

## VTCodeSimilarity-YaraGen

**Repository:** [https://github.com/arieljt/VTCodeSimilarity-YaraGen](https://github.com/arieljt/VTCodeSimilarity-YaraGen)

**Author:** arieljt

**Last Updated:** 2020-10-07

**Description:** Yara rule generator using VirusTotal code similarity feature `code-similar-to:`

---

## yaml2yara

**Repository:** [https://github.com/nccgroup/yaml2yara](https://github.com/nccgroup/yaml2yara)

**Author:** nccgroup

**Last Updated:** 2020-02-03

**Description:** Released as open source by NCC Group Plc - http://www.nccgroup.com/

---

## YARA

**Repository:** [https://github.com/anyrun/YARA](https://github.com/anyrun/YARA)

**Author:** anyrun

**Last Updated:** 2025-11-01

**Description:** This repository contains a curated collection of YARA rules designed to detect a wide range of malware families, phishing campaigns, and other malicious artifacts.

---

## YARA-Matches-Correspondance-Array

**Repository:** [https://github.com/m0n4/YARA-Matches-Correspondance-Array](https://github.com/m0n4/YARA-Matches-Correspondance-Array)

**Author:** m0n4

**Last Updated:** 2022-02-06

**Description:** YMCA is designed for threat hunters, incident responders, security analysts or anyone writing detection or hunting rules. This tool focuses exclusively on the strings section of the rules. It visually presents the correspondences between a ruleset and a collection of samples.

---

## yara-mode

**Repository:** [https://github.com/binjo/yara-mode](https://github.com/binjo/yara-mode)

**Author:** binjo

**Last Updated:** 2022-08-17

**Description:** No README found

---

## yara-parser

**Repository:** [https://github.com/Northern-Lights/yara-parser](https://github.com/Northern-Lights/yara-parser)

**Author:** Northern-Lights

**Last Updated:** 2022-12-20

**Description:** `yara-parser` is a Go library for manipulating YARA rulesets.  Its key feature is that it uses the same grammar and lexer files as the original libyara to ensure that lexing and parsing work exactly like YARA.  The grammar and lexer files have been modified to fill Go data structures for ruleset ...

---

## YARA-PE-Features

**Repository:** [https://github.com/g-les/YARA-PE-Features](https://github.com/g-les/YARA-PE-Features)

**Author:** g-les

**Last Updated:** 2021-10-07

**Description:** Slides from Cyber Defense Summit 2021 by Connor McLaughlin and Greg Lesnewich of Recorded Future

---

## YARA-Performance-Guidelines

**Repository:** [https://github.com/Neo23x0/YARA-Performance-Guidelines](https://github.com/Neo23x0/YARA-Performance-Guidelines)

**Author:** Neo23x0

**Last Updated:** 2025-02-11

**Description:** Writing efficient YARA rules is essential for maintaining fast and accurate scanning performance. This guide provides key principles and best practices to help you optimize your rules, reduce unnecessary computation, and avoid common pitfalls. It incorporates insights from industry experts, inclu...

---

## yara-procdump-python

**Repository:** [https://github.com/google/yara-procdump-python](https://github.com/google/yara-procdump-python)

**Author:** google

**Last Updated:** 2017-11-02

**Description:** A Python extension to wrap the Yara process memory access API.

---

## Yara-Repo

**Repository:** [https://github.com/Fadavvi/Yara-Repo](https://github.com/Fadavvi/Yara-Repo)

**Author:** Fadavvi

**Last Updated:** 2023-08-26

**Description:** A script to collect (the most famous) Yara rules from more than 150 free resources.

---

## yara-rules

**Repository:** [https://github.com/deadbits/yara-rules](https://github.com/deadbits/yara-rules)

**Author:** deadbits

**Last Updated:** 2020-03-25

**Description:** Collection of YARA signatures from recent malware research

---

## Yara-rules

**Repository:** [https://github.com/bartblaze/Yara-rules](https://github.com/bartblaze/Yara-rules)

**Author:** bartblaze

**Last Updated:** 2026-01-28

**Description:** A repo containing some of my privately developed Yara rules.

---

## Yara-Rules

**Repository:** [https://github.com/advanced-threat-research/Yara-Rules](https://github.com/advanced-threat-research/Yara-Rules)

**Author:** advanced-threat-research

**Last Updated:** 2025-03-18

**Description:** Repository of YARA rules to accompany the Trellix ATR blogposts & investigations

---

## YARA-rules

**Repository:** [https://github.com/fboldewin/YARA-rules](https://github.com/fboldewin/YARA-rules)

**Author:** fboldewin

**Last Updated:** 2023-03-25

**Description:** Some YARA rules i will add from time to time

---

## yara-rules-public

**Repository:** [https://github.com/jipegit/yara-rules-public](https://github.com/jipegit/yara-rules-public)

**Author:** jipegit

**Last Updated:** 2019-04-29

**Description:** CC Attribution 4.0 International (CC BY 4.0)

---

## yara-rust

**Repository:** [https://github.com/Hugal31/yara-rust](https://github.com/Hugal31/yara-rust)

**Author:** Hugal31

**Last Updated:** 2025-11-19

**Description:** Bindings for the [Yara library from VirusTotal](https://github.com/VirusTotal/yara).

---

## yara-signator

**Repository:** [https://github.com/fxb-cocacoding/yara-signator](https://github.com/fxb-cocacoding/yara-signator)

**Author:** fxb-cocacoding

**Last Updated:** 2022-04-11

**Description:** Automatic YARA rule generation for malware repositories. Currently used to build YARA signatures for Malpedia (https://malpedia.caad.fkie.fraunhofer.de) and limited to x86/x86-64 executables and memory dumps for Linux, macOS and Windows.

---

## YARA-Signatures

**Repository:** [https://github.com/jeFF0Falltrades/YARA-Signatures](https://github.com/jeFF0Falltrades/YARA-Signatures)

**Author:** jeFF0Falltrades

**Last Updated:** 2024-09-19

**Description:** A collection of my public YARA signatures for various malware families.

---

## YARA-sort

**Repository:** [https://github.com/horsicq/YARA-sort](https://github.com/horsicq/YARA-sort)

**Author:** horsicq

**Last Updated:** 2026-02-12

**Description:** More info: https://n10info.blogspot.com/2019/10/nfd-sort.html

---

## YARA-Style-Guide

**Repository:** [https://github.com/Neo23x0/YARA-Style-Guide](https://github.com/Neo23x0/YARA-Style-Guide)

**Author:** Neo23x0

**Last Updated:** 2024-02-17

**Description:** A specification and style guide for YARA rules

---

## Yara-Unprotect

**Repository:** [https://github.com/fr0gger/Yara-Unprotect](https://github.com/fr0gger/Yara-Unprotect)

**Author:** fr0gger

**Last Updated:** 2020-11-19

**Description:** This repository regroups the Yara Rules created for the Unprotect Project and for detecting the malware evasion techniques.

---

## yara-validator

**Repository:** [https://github.com/CIRCL/yara-validator](https://github.com/CIRCL/yara-validator)

**Author:** CIRCL

**Last Updated:** 2020-09-05

**Description:** Validates yara rules and tries to repair the broken ones.

---

## yara4pentesters

**Repository:** [https://github.com/DiabloHorn/yara4pentesters](https://github.com/DiabloHorn/yara4pentesters)

**Author:** DiabloHorn

**Last Updated:** 2018-01-31

**Description:** rules to identify files containing juicy information like usernames, passwords etc

---

## yara_rules

**Repository:** [https://github.com/f0wl/yara_rules](https://github.com/f0wl/yara_rules)

**Author:** f0wl

**Last Updated:** 2022-03-03

**Description:** A collection of yara rules that I created during analysis / for blog posts

---

## YARA_Rules

**Repository:** [https://github.com/MalGamy/YARA_Rules](https://github.com/MalGamy/YARA_Rules)

**Author:** MalGamy

**Last Updated:** 2023-01-27

**Description:** This project contains a collection of Yara rules that can be used to identify and classify various types of files.

---

## yara_scanner

**Repository:** [https://github.com/ace-ecosystem/yara_scanner](https://github.com/ace-ecosystem/yara_scanner)

**Author:** ace-ecosystem

**Last Updated:** 2022-02-19

**Description:** A wrapper around the [yara-python](https://github.com/VirusTotal/yara-python) project the provides the following capabilities.

---

## yara_tools

**Repository:** [https://github.com/matonis/yara_tools](https://github.com/matonis/yara_tools)

**Author:** matonis

**Last Updated:** 2019-01-12

**Description:** yara_tools

---

## yaradbg-backend

**Repository:** [https://github.com/DissectMalware/yaradbg-backend](https://github.com/DissectMalware/yaradbg-backend)

**Author:** DissectMalware

**Last Updated:** 2024-01-08

**Description:** YaraDbg is a free web-based Yara debugger to help security analysts to write hunting or detection rules with less effort and more confidence. By using YaraDbg, you can perform a thorough root-cause-analysis (RCA) on why some of your Yara rules did or did not match with a specific file. It can als...

---

## yaradbg-frontend

**Repository:** [https://github.com/DissectMalware/yaradbg-frontend](https://github.com/DissectMalware/yaradbg-frontend)

**Author:** DissectMalware

**Last Updated:** 2024-01-24

**Description:** YaraDbg is a free web-based Yara debugger to help security analysts to write hunting or detection rules with less effort and more confidence. By using YaraDbg, you can perform a thorough root-cause-analysis (RCA) on why some of your Yara rules did or did not match with a specific file. It can als...

---

## YaraFileCheckerLib

**Repository:** [https://github.com/k-sec-tools/YaraFileCheckerLib](https://github.com/k-sec-tools/YaraFileCheckerLib)

**Author:** k-sec-tools

**Last Updated:** 2022-02-07

**Description:** the library is designed to make it easier to check potentially malicious files and archives using YARA and make a decision about their harmfulness based on the weights of the detected rules

---

## YaraGen

**Repository:** [https://github.com/mrexodia/YaraGen](https://github.com/mrexodia/YaraGen)

**Author:** mrexodia

**Last Updated:** 2017-09-02

**Description:** Plugin for x64dbg to generate Yara rules from function basic blocks.

---

## YaraHunter

**Repository:** [https://github.com/deepfence/YaraHunter](https://github.com/deepfence/YaraHunter)

**Author:** deepfence

**Last Updated:** 2025-12-22

**Description:** Deepfence YaraHunter scans container images, running Docker containers, and filesystems to find indicators of malware. It uses a [YARA ruleset](https://github.com/deepfence/yara-rules) to identify resources that match known malware signatures, and may indicate that the container or filesystem has...

---

## yaralyzer

**Repository:** [https://github.com/michelcrypt4d4mus/yaralyzer](https://github.com/michelcrypt4d4mus/yaralyzer)

**Author:** michelcrypt4d4mus

**Last Updated:** 2026-01-31

**Description:** Visually inspect all of the regex matches (and their sexier, more cloak and dagger cousins, the [YARA](https://github.com/VirusTotal/yara-python) matches) found in binary data and/or text. See what happens when you force various character encodings upon those matched bytes. [With colors](https://...

---

## yaraMail

**Repository:** [https://github.com/kevthehermit/yaraMail](https://github.com/kevthehermit/yaraMail)

**Author:** kevthehermit

**Last Updated:** 2013-04-22

**Description:** Yara Scanner For IMAP Feeds and saved Streams

---

## yaramanager

**Repository:** [https://github.com/3c7/yaramanager](https://github.com/3c7/yaramanager)

**Author:** 3c7

**Last Updated:** 2022-11-02

**Description:** 

---

## YaraManager

**Repository:** [https://github.com/kevthehermit/YaraManager](https://github.com/kevthehermit/YaraManager)

**Author:** kevthehermit

**Last Updated:** 2020-03-09

**Description:** Web based Manager for Yara Rules

---

## yaraml_rules

**Repository:** [https://github.com/inv-ds-research/yaraml_rules](https://github.com/inv-ds-research/yaraml_rules)

**Author:** inv-ds-research

**Last Updated:** 2020-12-18

**Description:** *Questions, concerns, ideas, results, feedback appreciated, please email joshua.saxe@sophos.com*

---

## yaramod

**Repository:** [https://github.com/avast/yaramod](https://github.com/avast/yaramod)

**Author:** avast

**Last Updated:** 2026-01-25

**Description:** > :warning: Since version 4.x.x, yaramod is now focused on keeping compatibility with [YARA-X](https://github.com/VirusTotal/yara-x) and may not parse all YARA compatible rules anymore. It is planned to eventually decommission whole yaramod project in favor of YARA-X native parser. If you are int...

---

## yarAnalyzer

**Repository:** [https://github.com/Neo23x0/yarAnalyzer](https://github.com/Neo23x0/yarAnalyzer)

**Author:** Neo23x0

**Last Updated:** 2023-02-19

**Description:** Yara Rule Analyzer and Statistics

---

## yaraparser

**Repository:** [https://github.com/BitsOfBinary/yaraparser](https://github.com/BitsOfBinary/yaraparser)

**Author:** BitsOfBinary

**Last Updated:** 2020-07-12

**Description:** yaraparser

---

## YaraPcap

**Repository:** [https://github.com/kevthehermit/YaraPcap](https://github.com/kevthehermit/YaraPcap)

**Author:** kevthehermit

**Last Updated:** 2013-07-29

**Description:** Yara Scanner For IMAP Feeds and saved Streams

---

## yaraprocessor

**Repository:** [https://github.com/MITRECND/yaraprocessor](https://github.com/MITRECND/yaraprocessor)

**Author:** MITRECND

**Last Updated:** 2014-10-21

**Description:** Yaraprocessor

---

## YaraRules

**Repository:** [https://github.com/kevthehermit/YaraRules](https://github.com/kevthehermit/YaraRules)

**Author:** kevthehermit

**Last Updated:** 2016-02-07

**Description:** My Yara Rules Collection

---

## yarasafe

**Repository:** [https://github.com/lucamassarelli/yarasafe](https://github.com/lucamassarelli/yarasafe)

**Author:** lucamassarelli

**Last Updated:** 2020-02-25

**Description:** SAFE is a tool developed to create Binary Functions Embedding developed by Massarelli L., Di Luna G.A., Petroni F., Querzoni L. and Baldoni R. You can use SAFE to create your function embedding to use inside yara rules.

---

## yarascanner

**Repository:** [https://github.com/jheise/yarascanner](https://github.com/jheise/yarascanner)

**Author:** jheise

**Last Updated:** 2017-07-06

**Description:** Microservice for scanning files with yara

---

## yarastation

**Repository:** [https://github.com/NumLocK15/yarastation](https://github.com/NumLocK15/yarastation)

**Author:** NumLocK15

**Last Updated:** 2022-02-01

**Description:**  

---

## YaraSyntax

**Repository:** [https://github.com/nyx0/YaraSyntax](https://github.com/nyx0/YaraSyntax)

**Author:** nyx0

**Last Updated:** 2022-11-27

**Description:** 

---

## yaraVT

**Repository:** [https://github.com/deadbits/yaraVT](https://github.com/deadbits/yaraVT)

**Author:** deadbits

**Last Updated:** 2017-12-26

**Description:** You know you scan files with Yara anyways, why not give your findings back to the community? This script can scan a folder of samples against a provided Yara ruleset and optionally submit the matching Yara rule names to each files respective VirusTotal report as a comment.

---

