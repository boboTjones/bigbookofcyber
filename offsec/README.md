# Offensive Security (Red Team)

Penetration testing, exploitation, and red team tools for security assessment and adversarial simulation.

**Total Tools:** 46

---

## Active-Directory-Exploitation-Cheat-Sheet

**Repository:** [https://github.com/Integration-IT/Active-Directory-Exploitation-Cheat-Sheet](https://github.com/Integration-IT/Active-Directory-Exploitation-Cheat-Sheet)

**Author:** Integration-IT

**Last Updated:** 2020-11-14

**Description:** This repository contains a general methodology in the Active Directory environment. It is offered with a selection of quick commands from the most efficient tools based on Powershell, C, .Net 3.5 and .Net 4.5.

---

## Active-Directory-Penetration-Testing

**Repository:** [https://github.com/AD-Attacks/Active-Directory-Penetration-Testing](https://github.com/AD-Attacks/Active-Directory-Penetration-Testing)

**Author:** AD-Attacks

**Last Updated:** 2024-04-23

**Description:** description: >- Introduction to Active Directory Penetration Testing by RFS. Learn how to conquer Enterprise Domains. cover: .gitbook/assets/Active-Directory-Attacks.png coverY: -91.53292361720807

---

## Active-Directory-Penetration-Testing-and-Security

**Repository:** [https://github.com/DeanOfCyber/Active-Directory-Penetration-Testing-and-Security](https://github.com/DeanOfCyber/Active-Directory-Penetration-Testing-and-Security)

**Author:** DeanOfCyber

**Last Updated:** 2022-02-22

**Description:** Resources for AD penetration testing and security

---

## active-directory-pentest

**Repository:** [https://github.com/geeksniper/active-directory-pentest](https://github.com/geeksniper/active-directory-pentest)

**Author:** geeksniper

**Last Updated:** 2021-08-31

**Description:** * [Discovery](#discovery) * [Privilege Escalation](#privilege-escalation) * [Defense Evasion](#defense-evasion) * [Credential Dumping](#credential-dumping) * [Lateral Movement](#lateral-movement) * [Persistence](#persistence)

---

## Android-Exploits

**Repository:** [https://github.com/sundaysec/Android-Exploits](https://github.com/sundaysec/Android-Exploits)

**Author:** sundaysec

**Last Updated:** 2019-10-08

**Description:** A collection of android Exploits and guide on android exploitation

---

## android-penetration-testing-cheat-sheet

**Repository:** [https://github.com/ivan-sincek/android-penetration-testing-cheat-sheet](https://github.com/ivan-sincek/android-penetration-testing-cheat-sheet)

**Author:** ivan-sincek

**Last Updated:** 2026-02-12

**Description:** This is more of a checklist for myself. May contain useful tips and tricks. **Still need to add a lot of things.**

---

## Android-Pentesting-Checklist

**Repository:** [https://github.com/Hrishikesh7665/Android-Pentesting-Checklist](https://github.com/Hrishikesh7665/Android-Pentesting-Checklist)

**Author:** Hrishikesh7665

**Last Updated:** 2024-10-20

**Description:** **Welcome to the "Android App Penetration Testing Checklist" Repository!**

---

## awesome-arm-exploitation

**Repository:** [https://github.com/HenryHoggard/awesome-arm-exploitation](https://github.com/HenryHoggard/awesome-arm-exploitation)

**Author:** HenryHoggard

**Last Updated:** 2020-10-03

**Description:** A collection of awesome videos, articles, books, tools and resources about ARM exploitation.

---

## Awesome-Azure-Pentest

**Repository:** [https://github.com/Kyuu-Ji/Awesome-Azure-Pentest](https://github.com/Kyuu-Ji/Awesome-Azure-Pentest)

**Author:** Kyuu-Ji

**Last Updated:** 2023-01-31

**Description:** A curated list of useful tools and resources for penetration testing and securing Microsofts cloud platform **Azure**.

---

## Awesome-Pentest

**Repository:** [https://github.com/Al1ex/Awesome-Pentest](https://github.com/Al1ex/Awesome-Pentest)

**Author:** Al1ex

**Last Updated:** 2020-12-17

**Description:** * [Anonymity Tools](#anonymity-tools) * [Anti-virus Evasion Tools](#anti-virus-evasion-tools) * [Books](#books) * [Defensive Programming Books](#defensive-programming-books) * [Hacker's Handbook Series Books](#hackers-handbook-series-books)

---

## awesome-pentest

**Repository:** [git@github.com:boboTjones/awesome-pentest.git](git@github.com:boboTjones/awesome-pentest.git)

**Author:** boboTjones

**Last Updated:** 2026-01-25

**Description:** > A collection of awesome penetration testing and offensive cybersecurity resources.

---

## Awesome-Pentest

**Repository:** [https://github.com/Kc57/Awesome-Pentest](https://github.com/Kc57/Awesome-Pentest)

**Author:** Kc57

**Last Updated:** 2022-12-05

**Description:** > A list of awesome penetration testing tools and resources.

---

## awesome-pentest

**Repository:** [https://github.com/enaqx/awesome-pentest](https://github.com/enaqx/awesome-pentest)

**Author:** enaqx

**Last Updated:** 2026-02-11

**Description:** > A collection of awesome penetration testing and offensive cybersecurity resources.

---

## awesome-pentest-cheat-sheets

**Repository:** [https://github.com/ByteSnipers/awesome-pentest-cheat-sheets](https://github.com/ByteSnipers/awesome-pentest-cheat-sheets)

**Author:** ByteSnipers

**Last Updated:** 2024-06-27

**Description:** Collection of cheat sheets and check lists useful for security and pentesting. The list contains a huge list of very sorted and selected resources, which can help you to save a lot of time.

---

## Awesome-Red-Teaming

**Repository:** [https://github.com/yeyintminthuhtut/Awesome-Red-Teaming](https://github.com/yeyintminthuhtut/Awesome-Red-Teaming)

**Author:** yeyintminthuhtut

**Last Updated:** 2022-01-13

**Description:** List of Awesome Red Team / Red Teaming Resources

---

## C2-JARM

**Repository:** [https://github.com/cedowens/C2-JARM](https://github.com/cedowens/C2-JARM)

**Author:** cedowens

**Last Updated:** 2023-04-19

**Description:** A list of JARM hashes for different ssl implementations used by some C2 tools. Also adding other useful red team tools that use ssl (ex: EvilGinx2). Though I work on the red team side, I thought this would be a good thing to gather both to help blue teams who have the appropriate visibility with ...

---

## Covenant

**Repository:** [https://github.com/cobbr/Covenant](https://github.com/cobbr/Covenant)

**Author:** cobbr

**Last Updated:** 2021-04-21

**Description:** Covenant is a .NET command and control framework that aims to highlight the attack surface of .NET, make the use of offensive .NET tradecraft easier, and serve as a collaborative command and control platform for red teamers.

---

## DeimosC2

**Repository:** [https://github.com/DeimosC2/DeimosC2](https://github.com/DeimosC2/DeimosC2)

**Author:** DeimosC2

**Last Updated:** 2025-04-17

**Description:** Deimos is in: __DEPRECATED__

---

## Empire

**Repository:** [https://github.com/EmpireProject/Empire](https://github.com/EmpireProject/Empire)

**Author:** EmpireProject

**Last Updated:** 2019-07-31

**Description:** Empire is a post-exploitation framework that includes a pure-PowerShell2.0 Windows agent, and a pure Python 2.6/2.7 Linux/OS X agent. It is the merge of the previous PowerShell Empire and Python EmPyre projects. The framework offers cryptologically-secure communications and a flexible architectur...

---

## ios-penetration-testing-cheat-sheet

**Repository:** [https://github.com/ivan-sincek/ios-penetration-testing-cheat-sheet](https://github.com/ivan-sincek/ios-penetration-testing-cheat-sheet)

**Author:** ivan-sincek

**Last Updated:** 2025-11-07

**Description:** This is more of a checklist for myself. May contain useful tips and tricks. **Still need to add a lot of things.**

---

## ipv6-attack-detector

**Repository:** [https://github.com/mzweilin/ipv6-attack-detector](https://github.com/mzweilin/ipv6-attack-detector)

**Author:** mzweilin

**Last Updated:** 2020-07-30

**Description:** 6Guard (IPv6 attack detector)

---

## krackattacks-scripts

**Repository:** [https://github.com/vanhoefm/krackattacks-scripts](https://github.com/vanhoefm/krackattacks-scripts)

**Author:** vanhoefm

**Last Updated:** 2024-12-25

**Description:** This project contains scripts to test if clients or access points (APs) are affected by the KRACK attack against WPA2. For [details behind this attack see our website](https://www.krackattacks.com) and [the research paper](https://papers.mathyvanhoef.com/ccs2017.pdf).

---

## linux-exploit-suggester

**Repository:** [https://github.com/mzet-/linux-exploit-suggester](https://github.com/mzet-/linux-exploit-suggester)

**Author:** mzet-

**Last Updated:** 2026-02-11

**Description:** Quick download:

---

## linux-exploit-suggester-2

**Repository:** [https://github.com/jondonas/linux-exploit-suggester-2](https://github.com/jondonas/linux-exploit-suggester-2)

**Author:** jondonas

**Last Updated:** 2022-01-12

**Description:** Linux Exploit Suggester 2

---

## linux-soft-exploit-suggester

**Repository:** [https://github.com/belane/linux-soft-exploit-suggester](https://github.com/belane/linux-soft-exploit-suggester)

**Author:** belane

**Last Updated:** 2023-02-15

**Description:** Script to find exploits for all vulnerable software on the system, targeting software packages rather than just kernel vulnerabilities. It uses the [exploit database](https://gitlab.com/exploit-database/exploitdb) to assess the security of packages and search for exploits to help with privilege e...

---

## Linux_Exploit_Suggester

**Repository:** [https://github.com/PenturaLabs/Linux_Exploit_Suggester](https://github.com/PenturaLabs/Linux_Exploit_Suggester)

**Author:** PenturaLabs

**Last Updated:** 2014-05-19

**Description:** Linux_Exploit_Suggester

---

## Memcrashed-DDoS-Exploit

**Repository:** [https://github.com/649/Memcrashed-DDoS-Exploit](https://github.com/649/Memcrashed-DDoS-Exploit)

**Author:** 649

**Last Updated:** 2019-09-23

**Description:** * Author: [@037](https://twitter.com/037)

---

## mobile-pentest-toolkit

**Repository:** [https://github.com/ByteSnipers/mobile-pentest-toolkit](https://github.com/ByteSnipers/mobile-pentest-toolkit)

**Author:** ByteSnipers

**Last Updated:** 2025-10-30

**Description:** The MPT (Mobile Pentest Toolkit) is a **must-have** solution for your android penetration testing workflows. This tool allow you to automate security tasks and focus on security assessment without to know, where the tools are located and with parameters are required.

---

## offensive-tor-toolkit

**Repository:** [https://github.com/atorrescogollo/offensive-tor-toolkit](https://github.com/atorrescogollo/offensive-tor-toolkit)

**Author:** atorrescogollo

**Last Updated:** 2021-07-03

**Description:** **Offensive Tor Toolkit** is a series of tools that simplify the use of Tor for typical **exploitation and post-exploitation tasks**.

---

## PackerAttacker

**Repository:** [https://github.com/BromiumLabs/PackerAttacker](https://github.com/BromiumLabs/PackerAttacker)

**Author:** BromiumLabs

**Last Updated:** 2018-03-05

**Description:** The Packer Attacker is a generic hidden code extractor for Windows malware. It supports the following types of pacers:

---

## padding-oracle-attacker

**Repository:** [https://github.com/KishanBagaria/padding-oracle-attacker](https://github.com/KishanBagaria/padding-oracle-attacker)

**Author:** KishanBagaria

**Last Updated:** 2022-04-10

**Description:** CLI tool and library to execute [padding oracle attacks](https://en.wikipedia.org/wiki/Padding_oracle_attack) easily, with support for concurrent network requests and an elegant UI.

---

## PayloadsAllTheThings

**Repository:** [https://github.com/swisskyrepo/PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings)

**Author:** swisskyrepo

**Last Updated:** 2026-02-02

**Description:** A list of useful payloads and bypasses for Web Application Security. Feel free to improve with your payloads and techniques!

---

## pentest-wiki

**Repository:** [https://github.com/nixawk/pentest-wiki](https://github.com/nixawk/pentest-wiki)

**Author:** nixawk

**Last Updated:** 2020-09-22

**Description:** **Authors**: < [nixawk](https://github.com/nixawk) >, < [m1guelpf](https://github.com/m1guelpf) >, < [binarymist](https://github.com/binarymist) >

---

## pentest_lab

**Repository:** [https://github.com/oliverwiegers/pentest_lab](https://github.com/oliverwiegers/pentest_lab)

**Author:** oliverwiegers

**Last Updated:** 2025-06-17

**Description:** **I'm not actively developing on this anymore, but will fix bugs and look at issues and pull requests.** Help is appreciated :)

---

## php-exploit-scripts

**Repository:** [https://github.com/mattiasgeniar/php-exploit-scripts](https://github.com/mattiasgeniar/php-exploit-scripts)

**Author:** mattiasgeniar

**Last Updated:** 2023-02-23

**Description:** A collection of PHP exploit scripts, found when investigating hacked servers. These are stored for educational purposes and to test fuzzers and vulnerability scanners. Feel free to contribute.

---

## PoshC2

**Repository:** [https://github.com/nettitude/PoshC2](https://github.com/nettitude/PoshC2)

**Author:** nettitude

**Last Updated:** 2025-06-21

**Description:** PoshC2 is a proxy aware C2 framework used to aid penetration testers with red teaming, post-exploitation and lateral

---

## public-pentesting-reports

**Repository:** [https://github.com/juliocesarfort/public-pentesting-reports](https://github.com/juliocesarfort/public-pentesting-reports)

**Author:** juliocesarfort

**Last Updated:** 2025-11-24

**Description:** A repository containing public penetration test reports published by consulting firms and academic security groups.

---

## python-pentest-tools

**Repository:** [https://github.com/dloss/python-pentest-tools](https://github.com/dloss/python-pentest-tools)

**Author:** dloss

**Last Updated:** 2025-04-14

**Description:** Python tools for penetration testers

---

## redteam-plan

**Repository:** [https://github.com/magoo/redteam-plan](https://github.com/magoo/redteam-plan)

**Author:** magoo

**Last Updated:** 2017-08-23

**Description:** This document helps inform red team planning by contrasting against the very specific red team style described in [Red Teams](https://medium.com/starting-up-security/red-teams-6faa8d95f602). This method expresses several biases to optimize for blue team value and enthusiasm. It specifically avoid...

---

## Thick-Client-Pentest-Checklist

**Repository:** [https://github.com/Hari-prasaanth/Thick-Client-Pentest-Checklist](https://github.com/Hari-prasaanth/Thick-Client-Pentest-Checklist)

**Author:** Hari-prasaanth

**Last Updated:** 2022-10-26

**Description:** **OWASP Based Checklist  🌟🌟**

---

## trevorc2

**Repository:** [https://github.com/trustedsec/trevorc2](https://github.com/trustedsec/trevorc2)

**Author:** trustedsec

**Last Updated:** 2022-01-31

**Description:** Written by: Dave Kennedy (@HackingDave) Website: https://www.trustedsec.com

---

## Windows-Exploit-Suggester

**Repository:** [https://github.com/GDSSecurity/Windows-Exploit-Suggester](https://github.com/GDSSecurity/Windows-Exploit-Suggester)

**Author:** GDSSecurity

**Last Updated:** 2017-02-17

**Description:** DESCRIPTION This tool compares a targets patch levels against the Microsoft vulnerability database in order to detect potential missing patches on the target. It also notifies the user if there are public exploits and Metasploit modules

---

## Windows-Post-Exploitation

**Repository:** [https://github.com/emilyanncr/Windows-Post-Exploitation](https://github.com/emilyanncr/Windows-Post-Exploitation)

**Author:** emilyanncr

**Last Updated:** 2025-09-07

**Description:** Your contributions and suggestions are heartily♥ welcome. (✿◕‿◕). Please check the [Contributing Guidelines](CONTRIBUTING.md) for more details. This work is licensed under a [Creative Commons Attribution 4.0 International License](http://creativecommons.org/licenses/by/4.0/).

---

## Windows-Red-Team-Cheat-Sheet

**Repository:** [https://github.com/morph3/Windows-Red-Team-Cheat-Sheet](https://github.com/morph3/Windows-Red-Team-Cheat-Sheet)

**Author:** morph3

**Last Updated:** 2022-04-18

**Description:** This readme is now outdated, using https://notes.morph3.blog

---

## WindowsExploits

**Repository:** [https://github.com/abatchy17/WindowsExploits](https://github.com/abatchy17/WindowsExploits)

**Author:** abatchy17

**Last Updated:** 2017-05-21

**Description:** Mostly precompiled Windows exploits, largely forked from https://github.com/AusJock/Privilege-Escalation

---

## wordpress-exploit-framework

**Repository:** [https://github.com/rastating/wordpress-exploit-framework](https://github.com/rastating/wordpress-exploit-framework)

**Author:** rastating

**Last Updated:** 2019-11-24

**Description:** 

---

